...
export function ExpenseModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onSubmit({ title, amount: parseFloat(amount), currency, category, paidBy });
    reset();
    onClose();
  }
      ...
      >
        ...
        <div className="mt-5 flex gap-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-lg border border-zinc-800 py-2 text-sm font-medium text-zinc-300 hover:bg-zinc-900"
          >
            Cancel
          </button>
          <button
            type="submit"
            className="flex-1 rounded-lg bg-emerald-500 py-2 text-sm font-semibold text-black hover:bg-emerald-400"
          >
            Submit for vote
          </button>
        </div>
