...
export async function voteOnExpense(input: {
  ...
  if (newStatus !== "pending" && expenseRow.status !== newStatus) {
    ...
    await logAuditEvent({
      ...
      description: `"${expenseRow.title}" was ${newStatus} by founder vote (${approveCount} approve / ${rejectCount} reject)`,
