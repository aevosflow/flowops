...
const EVENT_TYPE_COLORS: Record<string, "emerald" | "amber" | "sky" | "red"> = {
  ...
  secret_rotated: "amber",
  secret_created: "amber",
  compliance_change: "amber",
};

const FINANCIAL_TYPES = ["payout_distributed", "expense_approved", "expense_rejected", "expense_created", "project_created", "project_updated"];
const GOVERNANCE_TYPES = ["secret_accessed", "secret_rotated", "secret_created", "compliance_change"];

export function HistoryClient({ events }: { events: AuditEvent[] }) {
  const [tab, setTab] = useState<(typeof TABS)[number]["id"]>("financial");

  const filteredEvents = events.filter((event) =>
    tab === "financial" ? FINANCIAL_TYPES.includes(event.type) : GOVERNANCE_TYPES.includes(event.type)
  );
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge tone={EVENT_TYPE_COLORS[event.type]}>{event.type.replace(/_/g, " ")}</Badge>
                    </div>
