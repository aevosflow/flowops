...
export function ProjectsClient({ initialProjects }: { initialProjects: Project[] }) {
  ...
  const avgMargin =
    projectCount > 0 ? Math.round(projects.reduce((sum, p) => sum + p.margin, 0) / projectCount) : 0;
      ...
      {projectCount === 0 ? (
        ...
        <div className="grid gap-4">
          <div className="flex justify-end">
            <button
              onClick={() => setModalOpen(true)}
              className="inline-flex items-center gap-2 rounded-lg bg-emerald-500 px-3 py-1.5 text-sm font-semibold text-black transition-colors hover:bg-emerald-400"
            >
              <Plus className="h-4 w-4" />
              Add Project
            </button>
